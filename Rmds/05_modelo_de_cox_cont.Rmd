...
- A maior importância destas funõees diz respeito ao uso delas em técnicas gráficas para avaliar a adequação do modelo ajustado.
...
- Ou, como veremos na próxima aula, quando se deseja estimar a função de sobrevivência em um certo tempo $t$ especificado.
...
- Em particular, a \structure{suposição de riscos proporcionais}.
    + A violaçãao desta suposição pode acarretar sérios viéses na estimação dos coeficientes do modelo.
...
- É razoável construir este gra´fico para cada covariável incluída no estudo.
