...
df.leucemia <- read.table(file = here::here("dados", "leucemia.txt"),
                         header = TRUE)
...
mod0 <- coxph(Surv(time = tempos, event = cens) ~ idadec +
                zpesoc + pasc + vacc + leuinic,
...
df.leucemia$leuini.pas <- interaction(df.leucemia$leuinic, df.leucemia$pasc)

mod1 <- coxph(Surv(time = tempos, event = cens) ~ idadec + zpesoc + vacc + strata(leuini.pas),
              data = df.leucemia, method = "breslow")

mod1 <- coxph(Surv(time = tempos, event = cens) ~ idadec + zpesoc + pasc + vacc + strata(leuinic),
               data = df.leucemia, method = "breslow")
...
df.tmo$fasegr <- factor(df.tmo$fasegr)
...
mod1 <- coxph(
  Surv(time = inicio, time2 = fim, event = status) ~ idade + sexo + fasegr + deag + decr + recplaq,
